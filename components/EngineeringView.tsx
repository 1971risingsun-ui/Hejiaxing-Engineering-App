...
export default function EngineeringView({ 
  ...
  const handleImportCompletionPDF = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      const updatedProjects = projects.map(p => {
        if (p.id === targetProject.id) {
          const newReport: CompletionReport = {
            id: generateId(), date: rData.date, worker: rData.worker, items: rData.items, notes: rData.content || '', signature: '', timestamp: Date.now()
          };
    ...
    } catch (error: any) { alert('匯入失敗: ' + error.message); } finally { setIsProcessing(false); e.target.value = ''; }
  };
      ...
      const updatedProjects = projects.map(p => {
        if (p.id === targetProject.id) {
          const newItems: ConstructionItem[] = (rData.items || []).map((item: any) => ({
              id: generateId(), name: item.name, quantity: item.quantity, unit: item.unit, location: item.location || '', worker: rData.worker, assistant: rData.assistant || '', date: rData.date
          }));
          ...
          return { 
              ...p, constructionItems: [...filteredItems, ...newItems],
              reports: [...otherReports, {
                  id: generateId(), date: rData.date, weather: rData.weather || 'sunny', content: rData.content || '', reporter: currentUser.name, timestamp: Date.now(), worker: rData.worker, assistant: rData.assistant || ''
              }]
          };
        }
        return p;
  ...
  const handleImportConstructionExcel = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const personnelInfo = getSafeText(worksheet.getCell('B3')); 
      ...
      worksheet.eachRow((row, rowNumber) => {
          if (rowNumber >= 7 && rowNumber < 10) {
              ...
              if (name && name !== '無施工項目') {
                  items.push({
                      id: generateId(), name, quantity: getSafeText(row.getCell(3)), unit: getSafeText(row.getCell(4)), location: getSafeText(row.getCell(5)), worker, assistant, date: reportDate
                  });
              }
          }
      });
      const updatedProjects = projects.map(p => {
          if (p.id === targetProject.id) {
              ...
              return { 
                  ...p, constructionItems: [...filteredItems, ...items],
                  reports: [...otherReports, { id: generateId(), date: reportDate, weather, content: notes, reporter: currentUser.name, timestamp: Date.now(), worker, assistant }]
              };
          }
          return p;
      });
  ...
  const handleExportExcel = () => {
    try {
      const data = projects.map((p, idx) => ({
        '項次': idx + 1, '案件名稱': p.name, '客戶名稱': p.clientName, '類型': p.type === ProjectType.CONSTRUCTION ? '圍籬' : p.type === ProjectType.MODULAR_HOUSE ? '組合屋' : '維修', '地址': p.address, '狀態': p.status, '預約日期': p.appointmentDate, '報修日期': p.reportDate, '工程概要': p.description
      }));
  ...
  return (
    <>
      <ProjectList 
        ...
        onTranslateAll={onTranslateAllProjects} // 正確連結回呼
