...
const REMOTE_DB_URL = 'https://489csslt-ops.github.io/hejiaxing-Engineering-info/db.json';
  ...
  useEffect(() => {
    const restore = async () => {
      try {
        ...
        // 初次加載執行智慧合併 (Update not Overwrite)
  ...
  const renderSidebarContent = () => (
    <>
      ...
      <nav className="flex-1 px-4 space-y-2 overflow-y-auto no-scrollbar pb-10">
        ...
        {(isViewAllowed('purchasing_hub') || isViewAllowed('hr') || isViewAllowed('production') || isViewAllowed('equipment')) && (
          <>
        ...
        {isViewAllowed('users') && (
          <>
            ...
            <button onClick={() => { setView('users'); setSelectedProject(null); setIsSidebarOpen(false); }} className={`flex items-center gap-3 px-4 py-3 rounded-lg w-full transition-colors ${view === 'users' ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800'}`}><ShieldIcon className="w-4 h-4" /> <span className="font-medium">系統帳號設定</span></button>
          </>
        )}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
        ...
        <main className="flex-1 min-h-0 bg-[#f8fafc] pb-safe flex flex-col overflow-hidden">
          {view === 'update_log' ? (<AuditLogList logs={auditLogs} />) : 
           ...
           view === 'equipment' ? (<EquipmentModule onNavigate={setView} allowedViews={currentUser!.role === UserRole.ADMIN ? ['equipment_tools','equipment_assets','equipment_vehicles'] : (systemRules.rolePermissions?.[currentUser!.role || UserRole.WORKER]?.allowedViews || [])} />) :
